...
const availabilityConfig: Record<AvailabilityFilter, { label: string; helper: string; accent: string; icon: typeof BookOpen }> = {
  ...
  'for-sale': { label: 'A la venta', helper: 'E-book, IBD e impreso disponible', accent: 'bg-[#F5F2ED] text-[#6E645B]', icon: BookOpen },
...
const CatalogExplorer = () => {
  ...
  return (
    <section className="border-t border-border/70 bg-[linear-gradient(180deg,#fbf8f4_0%,#ffffff_22%,#ffffff_100%)] py-12 md:py-16">
      <div className="container mx-auto px-4">
        <div className="mb-8 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div className="max-w-3xl">
            <p className="font-body text-[11px] font-semibold uppercase tracking-[0.2em] text-primary">Todo el catálogo</p>
            <h2 className="mt-2 font-heading text-2xl font-bold text-foreground md:text-3xl">
              Encuentra una publicación por formato, autor, colección, temática o año
            </h2>
          ...
          <aside className="lg:sticky lg:top-[112px]">
            <div className="overflow-hidden rounded-[28px] border border-border bg-card shadow-[0_20px_60px_rgba(43,48,59,0.08)]">
              <div className="flex items-center justify-between border-b border-border px-5 py-5">
                ...
                <button onClick={resetFilters} className="font-body text-sm font-medium text-primary transition-colors hover:text-primary/80">
                  Limpiar
                </button>
              </div>

              <div className="border-b border-border px-5 py-5">
                <label className="mb-2 block font-body text-[11px] font-semibold uppercase tracking-[0.18em] text-muted-foreground">
                  Buscar publicación
                </label>
                <div className="flex items-center gap-3 rounded-full border border-border bg-background px-4 py-3 focus-within:border-primary">
                  ...
                  <input
                    ...
                    placeholder="Título, autor, ISBN, tema..."
              ...
              <FilterBlock title="Formato" open={openSections.formato} onToggle={() => setOpenSections(prev => ({ ...prev, formato: !prev.formato }))}>
                <div className="grid grid-cols-2 gap-3">
                  {(['ebook', 'ibd', 'printed', 'audiobook'] as BookFormat[]).map(format => {
                    ...
                    return (
                      <button
                        key={format}
                        onClick={() => setSelectedFormat(current => current === format ? 'all' : format)}
                        className={`relative rounded-[20px] border p-4 text-left transition-all hover:-translate-y-0.5 hover:shadow-md ${isSelected ? 'ring-1 ring-primary/35 shadow-sm' : ''} ${config.className}`}
                      >
                ...
                <p className="mt-3 font-body text-xs leading-relaxed text-muted-foreground">
                  E-book, IBD e impreso disponibles según la metadata comercial de cada obra.
                </p>
              </FilterBlock>
                  ...
                  {(Object.keys(availabilityConfig) as AvailabilityFilter[]).map(key => {
                    ...
                    return (
                      <button
                        ...
                        className={`flex w-full items-center gap-3 rounded-[20px] border px-4 py-3 text-left transition-all hover:border-primary/30 hover:bg-primary/5 ${isSelected ? 'border-primary/40 bg-primary/5' : 'border-border bg-background'}`}
                      >
                        <div className={`flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl ${config.accent}`}>
                        ...
                        <span className="inline-flex h-8 min-w-8 items-center justify-center rounded-full bg-muted px-2 font-body text-sm font-semibold text-foreground">
                          {count}
                        </span>
                      </button>
                    );
                ...
                <div className="mt-4 max-h-[260px] space-y-3 overflow-y-auto pr-1">
                  {filteredCollections.map(collection => (
                    <CheckRow
                      ...
                      label={collection}
                      count={collectionsBase.filter(book => book.collection === collection).length}
                      onToggle={() => toggleInList(collection, selectedCollections, setSelectedCollections)}
                    />
                  ))}
                </div>
              </FilterBlock>
                ...
                <div className="mt-4 max-h-[260px] space-y-3 overflow-y-auto pr-1">
                  {filteredAuthors.map(author => (
                    <CheckRow
                      ...
                      label={author}
                      count={authorsBase.filter(book => getBookAuthors(book).includes(author)).length}
                      onToggle={() => toggleInList(author, selectedAuthors, setSelectedAuthors)}
                    />
                  ))}
                </div>
              </FilterBlock>
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  {categories.map(category => (
                    <FilterChip
                      ...
                      label={category}
                      count={categoriesBase.filter(book => book.category === category).length}
                      onClick={() => toggleInList(category, selectedCategories, setSelectedCategories)}
                    />
                  ))}
                </div>
              </FilterBlock>
              ...
              <FilterBlock title="Año de publicación" open={openSections.anios} onToggle={() => setOpenSections(prev => ({ ...prev, anios: !prev.anios }))}>
                <div className="grid grid-cols-3 gap-3">
                  {years.map(year => (
                    <button
                      key={year}
                      onClick={() => toggleInList(year, selectedYears, setSelectedYears)}
                      className={`rounded-[16px] border px-3 py-4 text-center transition-colors ${selectedYears.includes(year) ? 'border-primary bg-primary/10 text-primary' : 'border-border bg-muted/40 text-foreground hover:border-primary/30'}`}
                    >
                ...
                <div className="space-y-3">
                  {sedes.map(sede => (
                    <CheckRow
                      ...
                      label={sede}
                      count={sedesBase.filter(book => book.sede === sede).length}
                      onToggle={() => toggleInList(sede, selectedSedes, setSelectedSedes)}
                    />
                  ))}
                </div>
              </FilterBlock>
...
const FilterChip = ({
  ...
  <button
    onClick={onClick}
    className={`inline-flex items-center gap-1 rounded-full px-3 py-2 font-body text-xs font-semibold transition-colors ${
      active ? (tone === 'primary' ? 'bg-primary text-primary-foreground' : 'bg-foreground text-background') : 'bg-muted text-muted-foreground hover:bg-primary/10 hover:text-primary'
    }`}
  >
    <span>{label}</span>
